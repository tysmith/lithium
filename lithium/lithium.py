...
def processOptions():
    ...
    if args.testcase is None:
        try:
            testcaseFilename = extra_args.pop() # can be overridden by --testcase in processOptions
        except IndexError:
            parser.print_help()
            return False
    else:
    ...
    if not isPowerOfTwo(minimizeMin) or not isPowerOfTwo(minimizeMax) or len(extra_args) < 1:
        parser.print_help()
        return False
